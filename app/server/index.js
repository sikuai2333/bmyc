...
app.get(
  ...
  const dimensionStmt = db.prepare('SELECT id,category,detail FROM dimensions WHERE person_id = ?');
  const formatted = people.map((person) => {
    const dims = dimensionStmt.all(person.id);
    return {
      ...applySensitiveMask(person, req.user),
      dimensions: maskDimensions(dims, req.user, person.id)
    };
  });
...
app.get(
  ...
  const dimensions = db.prepare('SELECT id,category,detail FROM dimensions WHERE person_id = ?').all(person.id);

  res.json({
    ...applySensitiveMask(person, req.user),
    dimensions: maskDimensions(dimensions, req.user, person.id)
  });
});
...
app.put('/api/personnel/:id/dimensions', authenticate, (req, res) => {
  ...
  if (!canEditDimensions(req.user, targetPersonId)) {
    return res.status(403).json({ message: '无权维护多维度数据' });
  }

  const { dimensions } = req.body;
  if (!Array.isArray(dimensions)) {
    return res.status(400).json({ message: '缺少有效的多维度数据' });
  }
  ...
  const updateDimensions = db.transaction(() => {
    db.prepare('DELETE FROM dimensions WHERE person_id = ?').run(targetPersonId);
    dimensions.forEach((dimension) => {
      insertDimension.run(targetPersonId, dimension.category, dimension.detail || '待补充');
    });
  });
  updateDimensions();

  const fresh = db.prepare('SELECT id,category,detail FROM dimensions WHERE person_id = ?').all(targetPersonId);
  logAction({
    actorId: req.user.id,
    action: 'update',
...
app.post('/api/import/dimensions', authenticate, (req, res) => {
  const { personId, dimensions } = req.body;
  ...
  const transaction = db.transaction(() => {
    dimensions.forEach((dimension) => {
      insertDimension.run(personId, dimension.category, dimension.detail || '待补充');
    });
  });
  transaction();
  logAction({
    ...
    detail: { count: dimensions.length }
  });
...
app.post(
  ...
  const deleteDimensions = db.prepare('DELETE FROM dimensions WHERE person_id = ?');
  const insertDimension = db.prepare('INSERT INTO dimensions (person_id, category, detail) VALUES (?,?,?)');
  ...
  const summary = db.transaction(() => {
    ...
    people.forEach((person) => {
      ...
      if (Array.isArray(person.dimensions)) {
        ...
        deleteDimensions.run(targetId);
        person.dimensions.forEach((dimension) => {
          insertDimension.run(targetId, dimension.category, dimension.detail || '待补充');
        });
      }
    });
    return stats;
...
app.get('/api/export/people', authenticate, requirePermission('export.excel'), (req, res) => {
  ...
  const dimensionStmt = db.prepare('SELECT category,detail FROM dimensions WHERE person_id = ?');
  ...
  const summary = db.transaction(() => {
    ...
    payload.forEach((person) => {
      ...
      if (existing) {
        ...
        deleteDimensions.run(existing.id);
        person.dimensions.forEach((dimension) => {
          insertDimension.run(existing.id, dimension.category, dimension.detail);
        });
        stats.updated += 1;
      } else {
        const newId = insertPerson.run(
        ...
        person.dimensions.forEach((dimension) => {
          insertDimension.run(newId, dimension.category, dimension.detail);
        });
        stats.created += 1;
      }
    });
